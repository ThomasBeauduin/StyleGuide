...
The actual source files of a software project are to be stored inside the `src` folder. 
Alternatively, you can put them into the `lib` (if you're developing a library), 
or into the `app` folder (if your application's source files are not supposed to be compiled).
...
Often it is beneficial to include some reference data, examples and documentation into the project.
These would be in markdown texts or executable notebooks documentation and stored into a `docs` folder.
